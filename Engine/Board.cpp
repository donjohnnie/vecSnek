...
Board::Board(Graphics & gfx, const GameSettings & gs)
	:
	gfx(gfx), dimension(gs.getTileSize()),
	width(gs.getBoardWidth()), height(gs.getBoardHeight()),
	contents(std::vector<CellContents>(width*height))
{
...
{
	assert(loc.y*width + loc.x < width*height);
...
void Board::SpawnContents(std::mt19937 & rng, const Snake& snake, CellContents contentsType)
{
	...
	assert(newLoc.y * width + newLoc.x < width*height);
